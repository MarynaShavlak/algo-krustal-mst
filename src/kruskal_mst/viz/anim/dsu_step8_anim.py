...
def _draw_state(ax, i):
    ...
    for n in NODES:
        if parent[n] == n:
            ...
            ax.annotate(f"{tag} {t('р=')}{rank[n]}", (x, y), xytext=(x, y + 0.40), ha="center",
                        fontsize=8, fontweight="bold",
                        color=FOUND_BORDER if st.get("found") == n else "#B5651D")

    ax.set_title(f"{t('Крок')} {i + 1}/{len(STATES)}:  {t(st['desc'])}", fontsize=10)
    ax.set_xlim(-0.3, 6.3); ax.set_ylim(0.2, 4.0); ax.set_axis_off()
    handles = [Patch(facecolor=C_NODE, edgecolor=ROOT_BORDER, linewidth=2, label=t("корінь")),
