...
C_NODE_EDGE = "#2C6E8F"; C_BASE = "#DADDE0"; C_MST = "#2E8B57"
C_REJECT = "#E63946"; C_CYCLE = "#F4A300"
...
def bc_cycle_step8(G, pos=POS):
    ...
    nx.draw_networkx_nodes(G, pos, ax=ax, node_size=820, node_color="#A8D8EA",
                           edgecolors=C_NODE_EDGE, linewidths=1.8)
    nx.draw_networkx_labels(G, pos, ax=ax, font_size=12, font_weight="bold", font_color="#15384A")
    nx.draw_networkx_edges(G, pos, ax=ax, edgelist=base, edge_color=C_BASE, width=1.5)
    nx.draw_networkx_edges(G, pos, ax=ax, edgelist=green, edge_color=C_MST, width=3.0)
    nx.draw_networkx_edges(G, pos, ax=ax, edgelist=list(cyc), edge_color=C_CYCLE, width=4.0)
    ...
    ax.legend(handles=[
        ...
        Line2D([0], [0], color=C_CYCLE, lw=4, label="наявний шлях B→E→C"),
