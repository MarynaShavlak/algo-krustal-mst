...
def compare_has_path_vs_dsu(G, pos=POS):
    ...
    node_colors = [C_CONSIDER if n in ("B", "C") else (C_VISIT if n in visit_num else C_NODE)
                   for n in G.nodes()]
    ...
    nx.draw_networkx_edges(G, pos, ax=axL, edgelist=non_forest, edge_color=C_FAINT_DOT, width=1.2, style="dotted")
    nx.draw_networkx_edges(G, pos, ax=axL, edgelist=forest_only, edge_color=C_FOREST, width=2.2)
    nx.draw_networkx_edges(G, pos, ax=axL, edgelist=path_edges, edge_color=C_CONSIDER, width=4.0)
    ...
    dcolors = [C_MST if n == "A" else (C_CONSIDER if n in ("B", "C") else C_NODE) for n in D.nodes()]
    ...
    nx.draw_networkx_edges(D, DPOS, ax=axR, edgelist=find_edges, edge_color=C_CONSIDER, width=3.4,
