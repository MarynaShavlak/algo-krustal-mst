...
def exchange_argument(G, pos=POS_EX):
    ...
    panel(axL, green=[("A", "B"), ("C", "E"), ("B", "E"), ("D", "F"), ("E", "G")],
          red=("B", "D"), dashed=("A", "D"),
          title="Інше дерево T* (вага 43): додаємо A\u2013D -> цикл A\u2013B\u2013D",
          wlabels=[("A", "B"), ("B", "D"), ("A", "D")])
    axL.annotate("цикл A\u2013B\u2013D", (2.4, 3.0), fontsize=10, color="#444", style="italic")
    axL.annotate("+ A\u2013D (5): легше,\nКраскал обрав його", (pos["A"][0], pos["A"][1]),
                 xytext=(4.3, 4.0), fontsize=9.5, color=C_ADD, fontweight="bold", ha="left")
    axL.annotate("B\u2013D (9): найважче в циклі\n-> прибираємо", (pos["B"][0] + 1.5, 2.8),
    ...
    panel(axR, green=[("A", "D"), ("A", "B"), ("C", "E"), ("B", "E"), ("D", "F"), ("E", "G")],
          title="Після обміну (B\u2013D -> A\u2013D) = дерево Краскала T (вага 39)",
          wlabels=[("A", "D"), ("A", "B")])
    axR.annotate("B\u2013D більше немає", (2.4, 2.85), xytext=(0.2, 1.6), fontsize=9.5,
                 color="#888", ha="left", arrowprops=dict(arrowstyle="->", color="#bbb", lw=1.3))

    fig.legend(handles=[
        Line2D([0], [0], color=C_TREE, lw=3.2, label="ребро дерева"),
        Line2D([0], [0], color=C_ADD, lw=3.2, ls="--", label="додаємо (e = A\u2013D, легше)"),
        Line2D([0], [0], color=C_REMOVE, lw=3.2, label="прибираємо (f = B\u2013D, важче)"),
        Line2D([0], [0], color=C_FAINT, lw=2.2, label="інші ребра графа"),
    ], loc="lower center", ncol=4, fontsize=9, frameon=False, bbox_to_anchor=(0.5, 0.0))
    fig.suptitle("Аргумент обміну: важче ребро B\u2013D міняємо на легше A\u2013D — вага падає 43 -> 39",
