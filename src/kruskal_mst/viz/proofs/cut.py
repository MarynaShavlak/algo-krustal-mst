...
def cut_property(G, S=("A", "B", "C", "D", "E"), pos=POS_CUT):
    ...
    ax.annotate(t("РОЗРІЗ"), (5.0, cut_y), xytext=(4.5, cut_y + 0.18),
    ...
    for txt in nx.draw_networkx_edge_labels(G, pos, ax=ax, edge_labels=lab_in,
    ...
    ax.text(4.3, 3.9, t("Група A"), fontsize=11.5, color="#15384A", fontweight="bold")
    ax.text(-0.7, -0.45, t("Група B = {F, G}"), fontsize=11.5, color="#7a5b1e", fontweight="bold")

    lx = (pos[lightest[0]][0] + pos[lightest[1]][0]) / 2
    ly = (pos[lightest[0]][1] + pos[lightest[1]][1]) / 2
    w_light = G[lightest[0]][lightest[1]]["weight"]
    ax.annotate(t("найлегше ребро\nчерез розріз ->\nбезпечно в МОД"), (lx, ly), xytext=(4.55, 2.15),
                fontsize=9.5, color=C_LIGHT, fontweight="bold", ha="left", va="center",
                arrowprops=dict(arrowstyle="->", color=C_LIGHT, lw=1.6))

    ax.legend(handles=[
        Line2D([0], [0], color=C_CROSS, lw=3, label=t("ребра, що перетинають розріз")),
        Line2D([0], [0], color=C_LIGHT, lw=4,
               label=t("найлегше з них ({u}–{v}, вага {w})").format(u=lightest[0], v=lightest[1], w=w_light)),
        Line2D([0], [0], color=C_FAINT, lw=2.4, label="ребра всередині груп"),
    ], loc="upper left", fontsize=9, frameon=False)
    ax.set_title(t("Розріз: вершини поділено на дві групи; кольорові ребра їх з'єднують"), fontsize=12)
    ax.set_xlim(-0.9, 5.6)
